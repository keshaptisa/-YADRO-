...
- решение протестировано на `Python 3.13.1`;
- согласно `pyproject.toml`, минимально требуется `Python 3.11+`.

Сторонние библиотеки не используются.

Важно: команды запуска предполагают, что текущая рабочая директория — корень репозитория.
...
![Вывод сценария с избытком генерации](docs/images/surplus_output.png)
...
![Вывод сценария с дефицитом генерации](docs/images/shortage_output.png)
...
![Вывод запуска тестов](docs/images/tests_output.png)
...
Поле `kind` используется как декларативный тип генератора для читаемости входных данных и дополнительной валидации сценария. Алгоритм принятия решений опирается не на `kind`, а на реальные числовые профили `hourly_generation` и `cost_per_unit`.
...
В данной модели стоимость генератора считается линейно и зависит от его фиксированного почасового выпуска: `стоимость = hourly_generation * cost_per_unit`. То есть при включении генератор отдает весь заданный в профиле объем энергии за этот час. Это важное допущение, потому что именно оно позволяет независимо выбирать набор генераторов по стоимости без дополнительной оптимизации частичной загрузки.

Следствие этого допущения состоит в том, что система оплачивает весь почасовой выпуск включенного генератора, даже если фактически потребителям в этот час нужен немного меньший объем энергии. Именно поэтому в отчете возможны часы, где `generated_energy` немного больше, чем `served_energy`.
...
Для корректной работы динамического программирования при дробных значениях спроса и генерации в реализации используется масштабирование значений до целых чисел. Количество знаков после запятой определяется автоматически для каждого часа, после чего все вычисления в динамике выполняются в целочисленном пространстве. Это позволяет избежать нестабильности `float`-ключей и сохранить точность для разумных десятичных значений во входных данных.

Дополнительно сами почасовые планы в модели результата хранятся в неизменяемом виде. Это сделано специально, чтобы зафиксировать рассчитанное расписание как готовый артефакт симуляции и исключить случайные мутации после расчета.
...
- проверка конкретной итоговой стоимости для одного из встроенных сценариев;
- проверка детерминированности симуляции и текстового отчета;
- интеграционный тест полного пайплайна `load -> simulate -> report`;
- проверка сценария с дробными значениями спроса и генерации;
