...
![Вывод сценария с избытком генерации](docs/images/shortage_output.jpg)
...
![Вывод сценария с дефицитом генерации](docs/images/surplus_output.jpg)
...
1. перебираются все подмножества генераторов, так как их в постановке немного;
2. для каждого часа определяется максимально доступная генерация;
...
Идея улучшения по сравнению с полным перебором потребителей состоит в том, что подмножества потребителей не перебираются напрямую. Вместо этого используется динамическое программирование, которое существенно лучше масштабируется при росте числа потребителей и при этом сохраняет точность результата для текущей постановки.

Такой подход особенно естественен здесь, потому что при дефиците мощности задача по сути сводится к варианту рюкзака: нужно выбрать набор потребителей в пределах доступной мощности так, чтобы сначала максимизировать их количество, а затем максимизировать суммарный покрытый спрос.
...
С точки зрения асимптотики итоговый подход выглядит так: по генераторам выполняется перебор всех подмножеств, что допустимо из-за их малого количества в постановке, а по потребителям используется динамическое программирование по достижимым объемам покрытия вместо полного перебора всех подмножеств. Для учебного масштаба задачи это хороший компромисс между точностью, понятностью реализации и качеством алгоритмической идеи.
